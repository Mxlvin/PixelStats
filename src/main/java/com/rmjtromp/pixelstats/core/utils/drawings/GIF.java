...
public class GIF implements Drawable<Integer>, Sizeable<Integer> {
	
	private int frame = 0;
	private boolean loop = true;
	private final List<Frame> frames = new ArrayList<>();
	
	public GIF(InputStream input) throws IOException {
...
//		        int height = imageDescriptor != null && imageDescriptor.hasAttribute("imageHeight") ? Integer.parseInt(imageDescriptor.getAttribute("imageHeight")) : 0;
	    ...
	    if(frames.isEmpty()) throw new RuntimeException("GIF doesn't have any frames.");
	}

	@Override
	public GIF setSize(@NotNull Size<Integer> size) {
	...
	public GIF setLoop(boolean loop) {
		this.loop = loop;
		return this;
	}
